    ...
    export const SendFormData = (linkForm) =>{
      ...
      const form = document.querySelectorAll("#sendForm input");
      ...
      function formLaunch(e) {
       ...
       if(inputName.getAttribute('class') === 'valid' && inputPhone.getAttribute('class') === 'valid' && inputMail.getAttribute('class') === 'valid' && statusForm === 0){
        ...
        Data.append('mail',inputMail.value.trim().toLowerCase())
      ...
      function massage(mess) {
        ...
        setTimeout(() => {
          document.querySelector(
            ".titleFormMassage"
          ).innerHTML = `Запишіться <span>безкоштовно</span> та отримайте подарунок`;
        }, 3000);
      }
      ...
      function validateName(name) {
        if (name.id === "name") {
          ...
          if (name.value === "") {
            inValidField(name);
            statusForm++;
            massage(`Поле <span>ім'я та прізвище </span> не 
              можуть бути порожніми!`);
            
          } else if (!RegExpName.test(name.value)) {
            inValidField(name);
            statusForm++;
            massage(`Введено <span>некоректні символи</span>!`);
      ...
      function validateEmail(mail) {
        if (mail.id === "mail") {
          let RegExpName = /^[a-z0-9]+@[a-z]+\.[a-z]{2,3}$/i;
          if (mail.value === "") {
            inValidField(mail);
            statusForm++;
            massage(`Поле <span>електронна пошта </span> не 
              можуть бути порожніми!`);
